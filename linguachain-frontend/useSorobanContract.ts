...
// Adjust these to match your deployment (see CD workflow for the testnet
// contract ID that gets written out on each deploy).
const CONTRACT_ID =
  import.meta.env.VITE_SOROBAN_CONTRACT_ID ?? "REPLACE_WITH_CONTRACT_ID";
...
export interface UseSorobanContractResult {
  isLoading: boolean;
  error: string | null;
  submitScore: (score: number) => Promise<string | null>;
  getCertificate: (
    userAddress: string,
    certId: number
...
/**
 * Hook exposing typed wrappers around the LinguaChain Soroban contract's
 * public functions: submit_score, get_certificate, get_user_cert_count.
 *
 * Write calls (submit_score) are simulated, signed via Freighter, and
 * submitted to the network. Read-only calls are simulated locally and never
 * require a signature or a submitted transaction.
 */
export function useSorobanContract(): UseSorobanContractResult {
  ...
  /** Runs a read-only simulation and decodes the returned ScVal. */
  const view = useCallback(
    async (method: string, args: ReturnType<typeof nativeToScVal>[]) => {
      // A throwaway/source account is sufficient for a read-only simulation.
    ...
    async (userAddress: string, certId: number): Promise<Certificate | null> => {
      ...
      try {
        const args = [
          ...
          nativeToScVal(certId, { type: "u32" }),
        ];
        const raw = await view("get_certificate", args);
        if (!raw) return null;

        // Adjust field mapping here to match the contract's Certificate struct.
        return {
  ...
  const getUserCertCount = useCallback(
    async (userAddress: string): Promise<number | null> => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "get_user_cert_count failed."
        );
        return null;
      } finally {
        setIsLoading(false);
